...
#include <signal.h>
#include <sys/wait.h>
void createEveryoneElse(int chef_pid[6],int pusher_pid[4],char* wsaler_sem_name);
...
int main(int argc, char* argv[]){
	...
	int size=0;
	int totalDeserts=0;
	char **ingr = (char**)malloc(sizeof(char*)*(argc-2));
	for(int k=0;k<argc-2;k++){
		ingr[k]=(char*)malloc(sizeof(char)*3);
		strcpy(ingr[k],argv[k]);
	}
	size = atoi(argv[argc-2]);
	wsaler_sem_name=argv[argc-1];
	createEveryoneElse(chef_pid,pusher_pid,wsaler_sem_name);
	...
	if( (milk = sem_open(MILK_SEM_NAME,0600,0))==SEM_FAILED){
		perror("milk_sem_open");exit(EXIT_FAILURE);
	}
	if((flour = sem_open(FLOUR_SEM_NAME,0600,0))==SEM_FAILED){
		perror("flour_sem_open");exit(EXIT_FAILURE);
	}
	if((walnut = sem_open(WALNUT_SEM_NAME,0600,0))==SEM_FAILED){
		perror("walnuts_sem_open");exit(EXIT_FAILURE);
	}
	if((sugar = sem_open(SUGAR_SEM_NAME,0600,0))==SEM_FAILED){
		perror("sugar_sem_open");exit(EXIT_FAILURE);
	}
	if((wsaler_sem = sem_open(wsaler_sem_name,0600,0))==SEM_FAILED){
		perror("sugar_sem_open");exit(EXIT_FAILURE);
	}
...
void createEveryoneElse(int chef_pid[6],int pusher_pid[4],char* wsaler_sem_name){
	char* pusher_argv[]={0};
	char temp[20];
	char* child_argv[] = {"","",wsaler_sem_name,0};
	for(int i=0;i<6;i++){
		chef_pid[i] = fork();
		sprintf(temp,"%d",i);
		child_argv[1] = temp;
		if(i==0) child_argv[0]= MF_SEM_NAME;
		else if(i==1)child_argv[0]= MS_SEM_NAME;
		else if(i==2) child_argv[0]= MW_SEM_NAME;
		else if(i==3) child_argv[0]= WS_SEM_NAME;
		else if(i==4) child_argv[0]= FS_SEM_NAME;
		else if(i==5) child_argv[0]= FW_SEM_NAME;
