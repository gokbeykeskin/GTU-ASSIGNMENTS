...
void pvpHex(char hexBoard[12][12],int boardSize,bool checkedBefore[12][12]){
	bool gameFinished1LR=false,gameFinished1RL=false,gameFinished2LR=false,gameFinished2RL=false; //player 1 won(LEFT TO RIGHT - RIGHT TO LEFT), player 2 won(LEFT TO RIGHT - RIGHT TO LEFT).
	...
	while(gameFinished1LR==false && gameFinished1RL==false){
		do{ //player 1
			...
			gameFinished1LR=winCheckP1LR(hexBoard,0,0,boardSize,checkedBefore);                     //the reason I made 2 check conditions for each player is:
			gameFinished1RL=winCheckP1RL(hexBoard,boardSize-1,0,boardSize,checkedBefore);			//when there is a 'x'(or 'o') on left, 
		...
		do{  //player 2 (same as player 1.)
			cout << "Make your move(2st player): ";
			cin >> moveX >> moveY;
			moveX-=97;
			moveY-=1;
			if(moveX<0 || moveX>boardSize-1 ||moveY<0 ||moveY>boardSize-1){
			...
			else if(hexBoard[moveY][moveX]== '.'){
				hexBoard[moveY][moveX]='o';
				wrongMove=false;
			}
			else{
				cout << "This place is already occupied. Make another move."<<endl;
				wrongMove= true;
			}
			cout<<"\n";
			gameFinished2LR=winCheckP2LR(hexBoard,0,0,boardSize,checkedBefore);
			gameFinished2RL=winCheckP2RL(hexBoard,0,boardSize-1,boardSize,checkedBefore);
...
void pvCompHex(char hexBoard[12][12],int boardSize,bool checkedBefore[12][12]){
	...
	while(gameFinished2RL==false && gameFinished2LR==false){
		if(botFirstMove==true){  //if it's bot's first move or if bot is stuck somewhere, put 'o' to a random location.
			...
			}while(hexBoard[botMoveY][botMoveX]!='.');
		}
		...
		do{
			if(botFirstMove==false && startOver==false){
				...
				else if(hexBoard[moveY][moveX+1]=='.' && botFlag==false){ //if bot can't go down or leftdown, it blocks the player1. (if possible)
					botMoveX=moveX+1;
					botMoveY=moveY;
					botTempX=botMoveX;
					botTempY=botMoveY;
				...
				else if(botMoveX+1<boardSize && hexBoard[botMoveY][botMoveX+1]=='.'&&botFlag==false){ //if can't go down or block p1 go right
					botMoveX++;
					botFlag=false;
				}
				else if(botMoveX-1>=0 && hexBoard[botMoveY][botMoveX-1]=='.'&&botFlag==false){ //otherwise go left
					botMoveX--;
					botFlag=false;
				}
				...
				else if(botMoveY-1>=0 && hexBoard[botMoveY-1][botMoveX]=='.') //go directly up
				...
				if(hexBoard[botMoveY][botMoveX]== '.'){ //place the bot's move
					hexBoard[botMoveY][botMoveX]='o';
					wrongMove=false;
				}
		...
		do{
			...
			gameFinished1LR=winCheckP1LR(hexBoard,0,0,boardSize,checkedBefore);
			gameFinished1RL=winCheckP1RL(hexBoard,boardSize-1,0,boardSize,checkedBefore);
			if(gameFinished1LR==true || gameFinished1RL==true){
				cout << "PLAYER HAS WON. CONGRATS!!!"<<endl;
				return;
			}
			printBoard(hexBoard,boardSize);
